...
이 코드는 all_minmax라는 csv 파일에 EQP_ID_MODULE_NAME 열을 따로 임베딩하지 않고 나머지 변수열을 한 번에 합쳐서 학습에 사용돼는데, 나는 EQP_ID_MODULE_NAME열을 따로 nn.embedding해서 feature_embedding과 합쳐서 입력으로 사용하는 코드로 수정해줘.
추후에 나는 EQP_ID_MODULE_NAME의 임베딩 값을 시각화해서 확인할거야. 그리고 test 할때 attention map을 통해 각 변수 열이 얼마나 중요한지 순서대로 나열하고 싶은데, 미리 코드에 추가해줘.
중요한 과제이니 신중하게 코드 구현해줘.
